...
const GlobalRoadmapPage: React.FC = () => {
  ...
  const handleDragEnd = useCallback(
    ...
    [
      ...
      reorderInTree,
      findParent,
      saveGlobalOrderMutation,
      saveProjectOrderMutation,
      queryClient,
      currentUserId,
      viewMode,
    ]
            ...
            >
              ...
              >
                ...
                <Tooltip
                  title={item.name}
                  placement="top-start"
                  disableHoverListener={item.name.length < 35}
                >
                  <Typography
                    variant="body2"
                    sx={{
                      ...
                      {item.type === 'task' && (
                        <Typography
                          sx={{
                            ...
                            whiteSpace: 'nowrap',
                            textShadow:
                              (displayProgress || 0) > 50 ? '0 0 2px rgba(0,0,0,0.3)' : 'none',
                          }}
                        >
                          {displayProgress || 0}%
                        </Typography>
    ...
    <Box sx={{ p: 3 }}>
      ...
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
        <Box>
          <Typography
            variant="h5"
            sx={{ fontWeight: 800, letterSpacing: '-0.02em', color: '#1A1D29' }}
          >
            전체 Roadmap
          </Typography>
      ...
      {displayItems.length === 0 ? (
        ...
        <Box
          sx={{ borderRadius: 2, border: '1px solid #E5E7EB', overflow: 'hidden', bgcolor: '#fff' }}
        >
          {/* ── Header Row ── */}
