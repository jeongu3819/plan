...
import {
    Box, Typography, Paper, Chip, LinearProgress, IconButton,
    Divider, Tooltip, Drawer, List, ListItemButton,
    ListItemIcon, ListItemText, Switch, CircularProgress, Button,
} from '@mui/material';
...
import { format, differenceInDays, startOfMonth, endOfMonth, eachDayOfInterval, getDay, isSameDay, isToday as isDateToday, addMonths, subMonths } from 'date-fns';
import {
...
const HomePage: React.FC = () => {
    ...
    const resetToDefault = () => {
        ...
        saveMutation.mutate({ widgetIds: DEFAULT_VISIBLE, widgetOrder: DEFAULT_VISIBLE, gridLayouts: {} });
    };
    ...
    const handleDragEnd = (event: DragEndEvent) => {
        ...
        if (over && active.id !== over.id) {
            ...
            const fullNewOrder = newOrder.concat(
                widgetOrder.filter(id => !newOrder.includes(id))
            );
            queryClient.setQueryData(['layout', currentUserId], (old: any) => ({
                ...old,
                widgetOrder: fullNewOrder,
            }));
            saveMutation.mutate({ widgetIds: visibleWidgets, widgetOrder: fullNewOrder, gridLayouts: {} });
        }
    };
    ...
    const renderTaskList = (tasks: Task[] | undefined, emptyMsg: string) => {
        if (!tasks || tasks.length === 0) {
            return (
                <Box sx={{ textAlign: 'center', py: 3 }}>
                    <Typography variant="body2" color="textSecondary" sx={{ fontSize: '0.8rem' }}>{emptyMsg}</Typography>
                </Box>
            );
        }
        return tasks.slice(0, 5).map(task => (
            ...
            >
                <Box sx={{ width: 8, height: 8, borderRadius: '50%', bgcolor: statusColors[task.status] || '#6B7280', flexShrink: 0 }} />
                <Box sx={{ flexGrow: 1, minWidth: 0 }}>
                    <Typography variant="body2" sx={{
                        fontWeight: 600, fontSize: '0.82rem', overflow: 'hidden',
                    ...
                    {task.due_date && (
                        <Typography variant="caption" sx={{
                            color: differenceInDays(new Date(task.due_date), new Date()) < 0 ? '#EF4444' : '#9CA3AF',
                            fontSize: '0.7rem',
                        }}>
                ...
                <Chip
                    ...
                    sx={{
                        ...
                        bgcolor: task.priority === 'high' ? '#FEF2F2' : task.priority === 'low' ? '#F3F4F6' : '#EFF6FF',
                        color: task.priority === 'high' ? '#EF4444' : task.priority === 'low' ? '#6B7280' : '#3B82F6',
                    }}
                />
            </Box>
        ));
    ...
    const WidgetHeader: React.FC<{ def: WidgetDef }> = ({ def }) => (
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 2, px: 0.5 }}>
            ...
            <Typography variant="subtitle2" sx={{ fontWeight: 700, fontSize: '0.85rem', flexGrow: 1 }}>{def.title}</Typography>
        </Box>
    );
    ...
    const renderWidget = (widgetId: string) => {
        ...
        switch (widgetId) {
            case 'overview': {
                const statusFilterMap: Record<string, string> = { 'Total': 'all', 'In Progress': 'in_progress', 'Done': 'done', 'To Do': 'todo' };
                const allOverviewTasks: Task[] = [...(stats?.overdue || []), ...(stats?.upcoming || []), ...(stats?.my_tasks || [])];
                const uniqueOverviewTasks = allOverviewTasks.filter((t, i, arr) => arr.findIndex(x => x.id === t.id) === i);
                const filteredOverviewTasks = overviewFilter && overviewFilter !== 'all'
                ...
                return (
                    <>
                        ...
                        {statsLoading ? <Box sx={{ textAlign: 'center', py: 3 }}><CircularProgress size={20} /></Box> : (
                            <Box>
                                <Box sx={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 1.5, mb: 2 }}>
                                    {[
                                        ...
                                        <Box
                                            key={item.label}
                                            onClick={() => setOverviewFilter(prev => prev === statusFilterMap[item.label] ? null : statusFilterMap[item.label])}
                                            sx={{
                                                p: 1.5, borderRadius: 2, cursor: 'pointer',
                                                bgcolor: overviewFilter === statusFilterMap[item.label] ? `${item.color}10` : '#F9FAFB',
                                                border: overviewFilter === statusFilterMap[item.label] ? `2px solid ${item.color}` : '1px solid #F3F4F6',
                                                transition: 'all 0.15s',
                                                '&:hover': { bgcolor: `${item.color}08`, transform: 'translateY(-1px)' },
                                            }}
                                        >
                                            <Typography variant="caption" sx={{ color: '#9CA3AF', fontSize: '0.65rem', fontWeight: 500 }}>{item.label}</Typography>
                                            <Typography variant="h6" sx={{ fontWeight: 800, color: item.color, fontSize: '1.3rem' }}>{item.value}</Typography>
                                        </Box>
                                    ))}
                                </Box>
                                ...
                                {overviewFilter && (
                                    <Box sx={{ mt: 1 }}>
                                        <Divider sx={{ mb: 1 }} />
                                        <Typography variant="caption" sx={{ fontWeight: 700, fontSize: '0.7rem', color: '#6B7280', mb: 0.5, display: 'block' }}>
                                            {overviewFilter === 'all' ? 'All Tasks' : statusLabels[overviewFilter] || overviewFilter} ({filteredOverviewTasks.length})
                                        </Typography>
                                        {filteredOverviewTasks.length === 0 ? (
                                            <Typography sx={{ fontSize: '0.75rem', color: '#9CA3AF', py: 1, textAlign: 'center' }}>No tasks</Typography>
                                        ) : (
                                            filteredOverviewTasks.slice(0, calExpanded ? undefined : 5).map(task => (
                                                <Box
                                                    ...
                                                    sx={{
                                                        ...
                                                        '&:hover': { bgcolor: '#F3F4F6' }, transition: 'background 0.1s',
                                                    }}
                                                >
                                                    <Box sx={{ width: 7, height: 7, borderRadius: '50%', flexShrink: 0, bgcolor: statusColors[task.status] || '#6B7280' }} />
                                                    <Typography sx={{ fontSize: '0.75rem', fontWeight: 500, flexGrow: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                                                        {task.title}
                                                    </Typography>
                                                    {task.due_date && (
                                                        <Typography sx={{ fontSize: '0.6rem', color: '#9CA3AF', flexShrink: 0 }}>
                                        ...
                                        {filteredOverviewTasks.length > 5 && (
                                            <Typography
                                                onClick={(e) => { e.stopPropagation(); setCalExpanded(!calExpanded); }}
                                                sx={{ fontSize: '0.7rem', color: '#2955FF', fontWeight: 600, cursor: 'pointer', textAlign: 'center', py: 0.5, mt: 0.5, '&:hover': { textDecoration: 'underline' } }}
                                            >
                                ...
                                {(stats?.total || 0) > 0 && !overviewFilter && (
                                    <Box>
                                        <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 0.5 }}>
                                            <Typography variant="caption" sx={{ fontWeight: 600, fontSize: '0.7rem' }}>Completion</Typography>
                                            <Typography variant="caption" sx={{ fontWeight: 700, fontSize: '0.7rem', color: '#22C55E' }}>
            ...
            case 'overdue':
                return <><WidgetHeader def={def} />{renderTaskList(stats?.overdue, 'No overdue tasks 🎉')}</>;
            case 'upcoming':
                return <><WidgetHeader def={def} />{renderTaskList(stats?.upcoming, 'No upcoming tasks')}</>;
            case 'mytasks':
                return <><WidgetHeader def={def} />{renderTaskList(stats?.my_tasks, 'No tasks assigned to you')}</>;
            case 'projects':
                return (
                    <>
                        <WidgetHeader def={def} />
                        {(stats?.project_stats || []).length === 0 ? (
                            <Box sx={{ textAlign: 'center', py: 3 }}>
                                <Typography variant="body2" color="textSecondary" sx={{ fontSize: '0.8rem' }}>No projects yet</Typography>
                            </Box>
                            ...
                            <Box
                                ...
                                sx={{
                                    py: 1, px: 1.5, borderRadius: 1.5, cursor: 'pointer',
                                    '&:hover': { bgcolor: '#F3F4F6' }, transition: 'all 0.15s',
                                }}
                            >
                                <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 0.5 }}>
                                    <Typography variant="body2" sx={{ fontWeight: 600, fontSize: '0.82rem' }}>{p.name}</Typography>
                                    <Typography variant="caption" sx={{ color: '#9CA3AF', fontSize: '0.7rem' }}>{p.done}/{p.total}</Typography>
                                </Box>
                                <LinearProgress
                                    variant="determinate" value={p.progress}
                                    sx={{ height: 4, borderRadius: 2, bgcolor: '#E5E7EB', '& .MuiLinearProgress-bar': { bgcolor: '#2955FF', borderRadius: 2 } }}
                                />
                            </Box>
                        ))}
                    </>
            ...
            case 'calendar': {
                ...
                const allTasks: Task[] = [...(stats?.overdue || []), ...(stats?.upcoming || []), ...(stats?.my_tasks || [])];
                ...
                mDays.forEach(day => {
                    uniqueTasks.filter(t => t.due_date && isSameDay(new Date(t.due_date), day)).forEach(t => {
                    ...
                    <>
                        <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 1.5 }}>
                            <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
                                {React.cloneElement(def.icon as React.ReactElement, { sx: { fontSize: '1.1rem', color: '#2955FF' } })}
                                <Typography variant="subtitle2" sx={{ fontWeight: 700, fontSize: '0.9rem' }}>{def.title}</Typography>
                            </Box>
                            <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.3 }}>
                                <IconButton size="small" onClick={() => { setCalMonth(subMonths(calMonth, 1)); setSelectedDate(null); }} sx={{ p: 0.3 }}><ChevronLeftIcon sx={{ fontSize: '1.1rem' }} /></IconButton>
                                <Typography variant="caption" sx={{ fontWeight: 700, fontSize: '0.8rem', minWidth: 90, textAlign: 'center' }}>
                                    {format(calMonth, 'yyyy년 M월')}
                                </Typography>
                                <IconButton size="small" onClick={() => { setCalMonth(addMonths(calMonth, 1)); setSelectedDate(null); }} sx={{ p: 0.3 }}><ChevronRightIcon sx={{ fontSize: '1.1rem' }} /></IconButton>
                            </Box>
                        </Box>
                        {/* Weekday headers */}
                        <Box sx={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: 0, mb: 0.5 }}>
                            {WDAYS.map((d, i) => (
                                <Box key={i} sx={{ textAlign: 'center', fontSize: '0.7rem', fontWeight: 700, color: i === 0 ? '#EF4444' : i === 6 ? '#3B82F6' : '#9CA3AF', py: 0.3 }}>{d}</Box>
                            ))}
                        </Box>
                        {/* Day grid */}
                        <Box sx={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: '2px' }}>
                            {Array.from({ length: pad }).map((_, i) => <Box key={`p${i}`} sx={{ height: 38 }} />)}
                            {mDays.map(day => {
                                ...
                                return (
                                    <Tooltip
                                        key={dayNum}
                                        title={holiday || (dayTasks.length > 0 ? dayTasks.map(t => t.title).join(', ') : '')}
                                        arrow
                                        placement="top"
                                    >
                                        <Box
                                            ...
                                            sx={{
                                                ...
                                                cursor: dayTasks.length > 0 ? 'pointer' : 'default', position: 'relative',
                                                bgcolor: isSelected ? '#1E44CC' : isTd ? '#2955FF' : holiday ? '#FEF2F2' : 'transparent',
                                                '&:hover': { bgcolor: isSelected ? '#1E44CC' : isTd ? '#1E44CC' : holiday ? '#FEE2E2' : '#F3F4F6' },
                                        ...
                                        >
                                            <Typography sx={{
                                                ...
                                                color: isSelected || isTd ? '#fff' : holiday || isSun ? '#EF4444' : isSat ? '#3B82F6' : '#374151',
                                                lineHeight: 1,
                                            }}>{dayNum}</Typography>
                                            {holiday && (
                                                <Typography sx={{
                                                    fontSize: '0.45rem', color: isSelected || isTd ? '#fff' : '#EF4444',
                                                    fontWeight: 600, lineHeight: 1, mt: 0.3,
                                ...
                                {selectedDate && (
                                    <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 0.5 }}>
                                        ...
                                        <Typography
                                            ...
                                            sx={{ fontSize: '0.65rem', color: '#2955FF', cursor: 'pointer', fontWeight: 600, '&:hover': { textDecoration: 'underline' } }}
                                        >전체보기</Typography>
                                    </Box>
                                )}
                                <Box>
                                    {(calExpanded ? displayEvents : displayEvents.slice(0, showLimit)).map((ev, idx) => (
                                        <Box
                                            key={idx}
                                            onClick={() => openDrawer(ev.task)}
                                            sx={{
                                                display: 'flex', alignItems: 'center', gap: 1, py: 0.6, px: 0.5,
                                                borderRadius: 1, cursor: 'pointer',
                                                '&:hover': { bgcolor: '#F3F4F6' }, transition: 'background 0.1s',
                                            }}
                                        >
                                            <Box sx={{ width: 7, height: 7, borderRadius: '50%', flexShrink: 0, bgcolor: statusColors[ev.status] || '#6B7280' }} />
                                            <Typography sx={{ fontSize: '0.75rem', fontWeight: 500, color: '#374151', flexGrow: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                                                {ev.label}
                                            </Typography>
    ...
    return (
        <Box>
            ...
            <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 3 }}>
                <Box>
                    <Typography variant="h4" sx={{ fontWeight: 800, color: '#1A1D29', letterSpacing: '-0.03em' }}>
                        Dashboard
                    </Typography>
            ...
            >
                <Typography variant="h6" sx={{ fontWeight: 700, mb: 0.5 }}>Widgets</Typography>
                ...
                <List disablePadding>
                    {ALL_WIDGETS.map(w => (
                        ...
                        >
                            <ListItemIcon sx={{ minWidth: 36, color: visibleWidgets.includes(w.id) ? '#2955FF' : '#D1D5DB' }}>
                                {w.icon}
                            </ListItemIcon>
                            <ListItemText
                                primary={w.title}
