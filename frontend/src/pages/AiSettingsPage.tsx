...
const AiSettingsPage: React.FC = () => {
    ...
    const saveMutation = useMutation({
        mutationFn: () => api.saveAiSettings({ api_url: apiUrl, model_name: modelName, api_key: apiKey }),
        onSuccess: () => {
            ...
            <Paper sx={{ p: 3, borderRadius: 3, border: '1px solid #E5E7EB' }}>
                <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2.5 }}>
                                ...
                                endAdornment: (
                                    <InputAdornment position="end">
                                        <IconButton size="small" onClick={() => setShowKey(!showKey)}>
                                            {showKey ? <VisibilityOffIcon sx={{ fontSize: '1rem' }} /> : <VisibilityIcon sx={{ fontSize: '1rem' }} />}
                                        </IconButton>
                                    </InputAdornment>
                                ),
                            }}
                    ...
                    <Button
                        ...
                        sx={{
                            bgcolor: '#2955FF', textTransform: 'none', fontWeight: 700,
                            mt: 1, borderRadius: 2, alignSelf: 'flex-start',
