 ...
 * Also supports saving existing project structure as a personal template (future).
 */
...
import { PROJECT_TEMPLATES, ProjectTemplate } from './ZeroStateDashboard';
            ...
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, mb: 2.5 }}>
              ...
              <Box>
                ...
                <Typography variant="caption" sx={{ color: '#6B7280' }}>
                  {selected.defaultTasks.length}개 기본 Task가 자동 생성됩니다
                </Typography>
              </Box>
            </Box>
            <TextField
            ...
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 0.5 }}>
              {selected.defaultTasks.map((task, i) => (
                <Box key={i} sx={{ display: 'flex', alignItems: 'center', gap: 1, px: 1, py: 0.5 }}>
                  ...
                  <Typography variant="body2" sx={{ fontSize: '0.78rem', color: '#374151' }}>
                    {task.title}
                  </Typography>
                  {task.priority === 'high' && (
                    <Chip
                      label="High"
                      size="small"
                      sx={{ height: 14, fontSize: '0.5rem', bgcolor: '#FEF2F2', color: '#EF4444' }}
                    />
                  )}
                </Box>
              ))}
