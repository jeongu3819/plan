...
const WorkNoteModal: React.FC<WorkNoteModalProps> = ({ open, onClose, taskId, taskTitle, canEdit }) => {
    ...
    const createMut = useMutation({
        mutationFn: (data: { content: string; block_type: string; checked?: boolean; style?: any }) =>
    ...
    const toggleBold = (block: TaskActivity) => {
        const currentBold = block.style?.bold || false;
        updateMut.mutate({
            id: block.id,
            style: { ...block.style, bold: !currentBold },
        });
    };
    ...
    const toggleBlockType = (block: TaskActivity) => {
        const newType = block.block_type === 'checkbox' ? 'text' : 'checkbox';
        updateMut.mutate({
            id: block.id,
            block_type: newType,
            checked: false,
                    ...
                    >
                        ...
                        <Box sx={{
                            ...
                            {block.block_type === 'checkbox' ? (
                                <IconButton
                                    size="small"
                                    onClick={() => canEdit && updateMut.mutate({ id: block.id, checked: !block.checked })}
                                    disabled={!canEdit}
                                    sx={{ p: 0.3 }}
                                >
                                    {block.checked
                                ...
                                <DragIndicatorIcon sx={{ fontSize: '1rem', color: '#D1D5DB', mt: 0.2 }} />
                            )}
                        </Box>
                            ...
                            <TextField
                                ...
                                onBlur={e => {
                                    if (e.target.value !== block.content) {
                                        updateMut.mutate({ id: block.id, content: e.target.value });
                                    }
                                }}
                                onFocus={() => setFocusedBlockId(block.id)}
                                onKeyDown={e => canEdit && handleKeyDown(e, block, index)}
                                disabled={!canEdit}
                                InputProps={{
                        ...
                        {canEdit && (
                            ...
                            >
                                <Tooltip title={block.block_type === 'checkbox' ? '텍스트로 변환' : '체크박스로 변환'} arrow>
                                    <IconButton size="small" onClick={() => toggleBlockType(block)} sx={{ color: '#9CA3AF', p: 0.4 }}>
                                        {block.block_type === 'checkbox'
                                ...
                                <Tooltip title="굵게" arrow>
                                    <IconButton
                                        size="small"
                                        onClick={() => toggleBold(block)}
                                        sx={{
                                            color: block.style?.bold ? '#2955FF' : '#9CA3AF',
                                            p: 0.4,
                                        }}
                                    >
